...
const renderPokemon = async () => {
    ...
    for (const pokemonData of normalTypePokemon) {
        ...
        for (const typeName of typeNames) {
            ...
            typeImageName.src = `/FrontEnd/imagens/tipos-nomes/${typeName}_en.png`;
...
function updateLoginLink(username) {
  const loginLink = document.getElementById('loginEdit');
  
  if (username) {
    const userLink = document.createElement('a');
    userLink.href = '/frontend/paginas/settings/index.html';
    userLink.textContent = username.split(' ')[0];
